...
def parse_args() -> argparse.Namespace:
    ...
    check_parser = subparsers.add_parser("check-version", help="Check whether a version satisfies workflow gates.")
    check_parser.add_argument("--version", required=True)
    check_parser.add_argument("--json-out", help="Optional path for a machine-readable check report.")
    check_parser.add_argument(
        "--allow-no-submission",
        action="store_true",
        help="Allow CV-only versions with no submission artifact.",
    )
...
def append_ledger_row(
    ...
    insert_ledger_row(ledger_path, row)


def insert_ledger_row(ledger_path: Path, row: str) -> None:
    content = ledger_path.read_text(encoding="utf-8")
    lines = content.splitlines(keepends=True)
    section_start = None
    for index, line in enumerate(lines):
        if line.strip() == "## Recent Submission Records":
            section_start = index
            break
    ...
    if section_start is None:
        with open(ledger_path, "a", encoding="utf-8", newline="") as handle:
            handle.write("\n")
            handle.write(row)
        print("Warning: Recent Submission Records section not found; appended ledger row to file end.")
        return
    ...
    lines.insert(insert_at, row if row.endswith("\n") else f"{row}\n")
...
def check_file(path: Path, description: str) -> dict[str, object]:
    return {
        "name": description,
...
def skip_check(description: str, path: str, detail: str) -> dict[str, object]:
    return {
        "name": description,
        "path": path,
        "status": "skip",
...
def check_version(args: argparse.Namespace) -> None:
    target_dir = version_dir(args.version)
    version_submission = target_dir / "submission.csv"
    submissions_copy = PROJECT_ROOT / "submissions" / f"{args.version}.csv"
    validation_path = target_dir / "validation_summary.json"
    manifest_path = target_dir / "run_manifest.json"
    ledger_path = PROJECT_ROOT / "EXPERIMENT_LEDGER.md"

    checks: list[dict[str, object]] = [
        check_file(target_dir / "README.md", "version README"),
        check_file(target_dir / "config.yml", "version config"),
        check_file(manifest_path, "run manifest"),
        check_file(target_dir / "reproduce.ps1", "reproduce script"),
        check_file(validation_path, "validation summary"),
        check_file(target_dir / "code_snapshot" / "SHA256SUMS.txt", "code snapshot checksums"),
    ]

    if args.allow_no_submission:
        checks.append(
            skip_check(
                "version submission",
                relative_or_absolute(version_submission),
                "skipped because --allow-no-submission was set",
            )
        )
        checks.append(
            skip_check(
                "submissions copy",
                relative_or_absolute(submissions_copy),
                "skipped because --allow-no-submission was set",
            )
        )
    else:
        checks.extend(
            [
                check_file(version_submission, "version submission"),
                check_file(submissions_copy, "submissions copy"),
            ]
        )

    validation_status = "fail"
    validation_detail = "validation summary missing"
    if validation_path.exists():
        try:
            ...
            validation_status = "pass" if validation.get("status") in allowed_statuses else "fail"
            validation_detail = f"status={validation.get('status')}"
        except json.JSONDecodeError as exc:
            validation_detail = f"invalid json: {exc}"
    checks.append(
        {
            "name": "validation summary status",
            "path": relative_or_absolute(validation_path),
    ...
    if manifest_path.exists():
        try:
            ...
            required_manifest_keys = {"version", "experiment_type", "iteration_mode", "exact_command", "git_commit", "seed"}
            missing = sorted(required_manifest_keys - set(manifest))
            manifest_status = "pass" if not missing else "fail"
            manifest_detail = "required keys present" if not missing else f"missing keys: {missing}"
        except json.JSONDecodeError as exc:
            manifest_detail = f"invalid json: {exc}"
    checks.append(
        {
            "name": "manifest required keys",
            "path": relative_or_absolute(manifest_path),
    ...
    if args.allow_no_submission:
        manifest_submission = None
        if manifest_payload is not None:
            manifest_submission = manifest_payload.get("files", {}).get("submission")
        manifest_submission_status = "pass" if manifest_submission is None else "fail"
        checks.append(
            {
                "name": "manifest submission is null",
                "path": relative_or_absolute(manifest_path),
                "status": manifest_submission_status,
                "detail": "submission=null" if manifest_submission is None else f"submission={manifest_submission}",
            }
        )
        checks.append(
            skip_check(
                "submission copy hash match",
                f"{relative_or_absolute(version_submission)} | {relative_or_absolute(submissions_copy)}",
                "skipped because --allow-no-submission was set",
            )
        )
    else:
        hash_status = "fail"
        hash_detail = "submission files missing"
        if version_submission.exists() and submissions_copy.exists():
            version_hash = sha256_file(version_submission)
            copy_hash = sha256_file(submissions_copy)
            hash_status = "pass" if version_hash == copy_hash else "fail"
            hash_detail = "hashes match" if version_hash == copy_hash else f"version={version_hash}, copy={copy_hash}"
        checks.append(
            {
                "name": "submission copy hash match",
                "path": f"{relative_or_absolute(version_submission)} | {relative_or_absolute(submissions_copy)}",
                "status": hash_status,
                "detail": hash_detail,
            }
        )
    ...
    ledger_status = "fail"
    ledger_detail = "ledger missing"
    if ledger_path.exists():
        ledger_text = ledger_path.read_text(encoding="utf-8")
        ledger_status = "pass" if args.version in ledger_text else "fail"
        ledger_detail = "version found in ledger" if args.version in ledger_text else "version not found in ledger"
    checks.append(
        {
            "name": "ledger record",
            "path": relative_or_absolute(ledger_path),
    ...
    if args.json_out:
        write_json(resolve_path(args.json_out), report)

    print(f"Version check: {args.version} -> {status}")
    for check in checks:
        marker = {"pass": "PASS", "skip": "SKIP"}.get(str(check["status"]), "FAIL")
