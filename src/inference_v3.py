"""
推理脚本 V3 - Phase 2集成版本 (inference_v3.py)
================================================
Phase 2: 使用集成模型进行推理

改进：
1. 使用V3模型（Stacking/Weighted集成）
2. 支持多种集成策略的推理
3. 自动加载特征融合器
...
def predict_site_v3(
    ...
    """
    使用V3模型（集成）对单个Site进行推理

    返回：
        result_df: 包含[site_path_timestamp, floor, x, y]的DataFrame
    """
    # 加载模型
    model_path = model_dir / f"{site_id}_v3.pkl"
    ...
    site_model = load_site_model_v3(model_path)
...
def run_inference_v3(
    ...
    output_path: Path = _PROJECT_ROOT / "submission_v3.csv",
    ...
    # 只保留有V3模型的Sites
    available_models = {p.stem.replace('_v3', '') for p in model_dir.glob("*_v3.pkl")}
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output", type=str, default=str(_PROJECT_ROOT / "submission_v3.csv"),
        help="输出CSV文件路径",
    )
    parser.add_argument(
