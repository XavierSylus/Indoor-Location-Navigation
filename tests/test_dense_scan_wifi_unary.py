...
class DenseScanWifiUnaryTest(unittest.TestCase):
    ...
    def test_rejects_waypoint_outside_scan_prediction_range(self) -> None:
        with self.assertRaisesRegex(ValueError, "outside dense scan prediction coverage"):
            interpolate_predictions(np.array([1000, 3000]), np.zeros((2, 2)), np.array([500]))
